...
- Log hygiene: litellm was logging full `Params passed to completion()` payloads at DEBUG, dumping system prompts, room-memory excerpts, and the tool-definition manifest in plaintext to `backend.log`. `backend/app/main.py` now sets the litellm logger family to WARNING by default at import time and flips `litellm.set_verbose=False` and `litellm.suppress_debug_info=True` defensively. `SPARKBOT_LITELLM_LOG_LEVEL=debug` re-enables verbose for explicit troubleshooting.
- Skill dedup: `backend/skills/calendar_create_event.py` and `backend/skills/calendar_list_events.py` were shadowed by native definitions in `tools.py` and dropped at startup with "Duplicate tool definition ignored" warnings. Both files removed.
- Test cleanup: split `test_server_read_command_service_logs_uses_allowed_service` into Linux/Windows variants with platform skipif markers, fixing a long-standing Windows test failure unrelated to this release.
- Tests: 4 new (host_identity profile dispatch, toolchain_versions profile dispatch, unsupported-profile rejection wording, profile registry contains both). Full suite: 190 passed, 1 skipped.
...
- Improvement Proposals visible in Command Center: three new operator-only routes (`GET /spine/operator/improvement/proposals`, `POST .../approve`, `POST .../reject`) back a new "Improvement" tab in the Inspector at the bottom of Command Center. The panel lists every proposal `guardian_propose_improvement` has recorded with status / risk / suggested change / evidence and Approve / Reject controls. Approving records operator intent only — code, config, or workflow changes still require an explicit follow-up action. Status transitions stamp `approved_at`/`approved_by` or `rejected_at`/`rejected_by` for audit. Frontend client uses `apiFetch` so the cross-origin auth bridge from v1.6.70 covers it.
- Tests: 9 new for the durable correction lock (record, room scoping, intent scoping, idempotent hit count, persistence, clear, disabled-flag respect, missing-input no-op); 2 new for `improvement.update_proposal_status` (approve+reject roundtrips and invalid inputs); 158 services tests pass overall.
...
- Added the Phase 2 LIMA Robo OS bridge: authenticated robotics status/tools/command/emergency-stop endpoints, a chat `lima_robot_command` tool for natural-language replay/simulation control, command-contract audit records, and default blocking for real-hardware motion until Guardian runner handoff is complete.
- Added Docker/server documentation and an optional Compose override for OpenAI Codex subscription routing through a host `codex login --device-auth` ChatGPT session.

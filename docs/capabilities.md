...
Supported schedule strings are `every:<seconds>`, `daily:<HH:MM>` in UTC, `daily-local:<HH:MM>` in the host's local timezone, and `at:<ISO-8601 UTC datetime>` for one-shot runs. For a complete demo pack, see [Sparkbot Jarvis Demo Kit](./jarvis-demo-kit.md).

### Scheduled tools (suggested)

- `sparkbot_health_check` - built-in PC/server health report templates; read-only, app-only by default, optional Telegram/Discord/Slack delivery when configured
...
WHATSAPP_APP_ID=
WHATSAPP_APP_SECRET=
WHATSAPP_ALLOWED_PHONES=15551234567
...
```
Setup:
1. Token path: paste a fine-grained GitHub token in Controls. The token scopes define what Sparkbot can do.
2. SSH path: set `GITHUB_SSH_KEY_PATH`, or paste a private key in Controls so it is stored locally.
3. GitHub App path: set App ID, Installation ID, and private key for install-scoped access.
4. Set `GITHUB_ALLOWED_REPOS` to keep assistant work constrained to known repositories.
...
For `v1.6.81`, the backend, frontend, Tauri shell, README, public download page, and release note are all advanced together so the installer, runtime self-inspection, and GitHub Pages downloader tell the same version story. This release fixes a critical Windows bug and adds two desktop-housekeeping items driven by direct inspection of the running v1.6.79 install. (1) **Windows bridge breakage**: `backend/app/api/routes/chat/tools.py` did `import pwd` at module top, which is POSIX-only. The import is lazy (`tools.py` is loaded inside `stream_chat_with_tools` at chat time, not at app startup), so the main UI worked — but Telegram, Discord, WhatsApp, and GitHub bridges all funnel through `stream_chat_with_tools`, so every bridged chat failed on Windows with `ModuleNotFoundError: No module named 'pwd'`. Confirmed in the operator's `sparkbot-backend.log` on 2026-05-16 inside the Telegram traceback. Fix: import `pwd` inside `_uid_name`, return the raw uid as a Windows-safe fallback. (2) **Log rotation on launch**: `desktop_launcher.py` now renames `sparkbot-backend.log` → `sparkbot-backend.log.1` on startup if it exceeds 50 MB, so upgraders from v1.6.79 (which wrote DEBUG and routinely produced 600+ MB files) get a clean slate while preserving the previous run for one triage cycle. (3) **PyInstaller `_MEI*` cleanup**: stale extracted runtime dirs under `%APPDATA%\Sparkbot\pyi-runtime\` are pruned on each launch (keeps newest 2 as a safety margin, skips the live `sys._MEIPASS`). The user's install had 30+ accumulated `_MEI*` dirs at ~50 MB each (~1.5 GB) because PyInstaller's own cleanup-on-exit fails under abrupt termination (Tauri close, OS logout, crash). (4) **CI noise removed**: deleted `.github/workflows/add-to-project.yml`, leftover FastAPI template cruft that fired on every PR and failed because `secrets.PROJECTS_TOKEN` was never set in Sparkpit's repo. 2 new regression tests assert that `tools.py` imports cleanly when `pwd` is unavailable and that `_uid_name` degrades safely; 180 services + tools tests pass.
...
For `v1.6.74`, the backend, frontend, Tauri shell, README, public download page, and release note are all advanced together so the installer, runtime self-inspection, and GitHub Pages downloader tell the same version story. This release adds self-diagnostic profiles (`host_identity` returns hostname + current user + OS + ISO time; `toolchain_versions` probes python/git/node/npm/docker independently so missing tools don't abort the audit) to `server_read_command` and tightens the rejection wording so an enum miss reads as a parameter validation error rather than a Guardian policy denial — closing the "BLOCKED: I could not verify host/time/user/toolchain status" symptom from the user's self-diagnostic audit. The tool description was also updated so the LLM has a clear recipe ("call this with command=host_identity, command=toolchain_versions, and command=system_overview in sequence"). On the security side, litellm was logging full `Params passed to completion()` payloads at DEBUG — system prompts, room memory excerpts, tool definition manifests in plaintext — into `backend.log`. `app/main.py` now sets the litellm logger family to WARNING by default at import time and flips `litellm.set_verbose=False` plus `litellm.suppress_debug_info=True` defensively; operators can override with `SPARKBOT_LITELLM_LOG_LEVEL=debug` for explicit troubleshooting. Removed `backend/skills/calendar_create_event.py` and `backend/skills/calendar_list_events.py` which were shadowed by native definitions in `tools.py` and dropped at startup with "Duplicate tool definition ignored" warnings on every launch. Split the pre-existing Linux-only `test_server_read_command_service_logs_uses_allowed_service` into platform-specific variants with `pytest.skipif` markers, fixing a long-standing Windows test failure that pre-dated this release. 4 new tests (host_identity dispatch, toolchain_versions dispatch, unsupported-profile rejection wording, profile registry contains both). Full suite: 190 passed, 1 skipped.

For `v1.6.73`, the backend, frontend, Tauri shell, README, public download page, and release note are all advanced together so the installer, runtime self-inspection, and GitHub Pages downloader tell the same version story. This release is a one-line policy fix: `backend/app/services/guardian/policy.py:decide_tool_use` was returning `action="privileged"` (asking the operator to enter `/breakglass`) for tools with `requires_execution_gate=True` (`server_read_command`, `ssh_read_command`, `server_manage_service`, write-like `shell_run`, robot commands, browser writes) even when break-glass was already active — because the lower active-break-glass branch was unreachable past the gated-tool short-circuit. Surfaced when the user triggered break-glass, asked Sparkbot to run a self-diagnostic audit, and the bot replied "Live diagnostics are policy-blocked in this environment; all PowerShell checks were rejected" instead of actually running the diagnostics. Fixed by checking `is_privileged` (active break-glass session) before `is_operator` in the gated-tool branch and short-circuiting to `allow` with reason "Break-glass PIN session is active; execution gate is bypassed for this action." The existing operator-prompt path (PIN not yet entered) and deny path (non-operator without break-glass) stay intact. 4 new regression tests in `test_guardian_policy.py`. Full guardian/services suite: 183 passed.

For `v1.6.72`, the backend, frontend, Tauri shell, README, public download page, and release note are all advanced together so the installer, runtime self-inspection, and GitHub Pages downloader tell the same version story. This release implements both v1.6.71 improvement proposals (operator-approved via the new Improvement tab). **Autonomous-turn pacing** is a new sidecar at `backend/app/services/guardian/autonomous_turn_pacing.py` that records per-(room, agent) outcomes; on 4xx provider failures (e.g. MiniMax error 2013 from a tool-incompatible model) the next attempt sleeps `2^min(N, 6)` seconds capped at 64, and after 8 consecutive 4xx within a 5-minute sliding window the pair is paused; `rooms.py:_run_agent_turn` consults `should_skip` before dispatching and records `record_failure` / `record_success` on the existing failure / success paths; 5xx and connection errors stay on the existing immediate-retry path because those are transient infra. New operator routes `GET /api/v1/chat/spine/operator/autonomous-pauses` and `POST .../resume` back a new card in the Task Guardian tab that lists paused pairs (failure count, last 4xx status, pause reason) plus a separate "Backing off" section so the operator can watch the loop heal in real time. **Durable Guardian sidecar state** introduces `SPARKBOT_GUARDIAN_DATA_DIR` as the umbrella env var consumed by `improvement.py`, `correction_lock.py`, and `autonomous_turn_pacing.py`; `backend/desktop_launcher.py` points it at `%APPDATA%\Sparkbot\guardian-data` so proposals, correction locks, and pacing state survive desktop restarts instead of being wiped with the PyInstaller `_MEI*` temp dir (which is what `Path(__file__).resolve().parents[3]` resolves to under PyInstaller, and was the v1.6.71 default for both sidecars). `_migrate_guardian_sidecar_data` runs once at launcher startup and seeds the umbrella from the freshest non-empty pre-umbrella source (legacy `<DATA_DIR>/<feature>/<file>` path or the most-recent `_MEI*/data/<feature>/<file>`), idempotent — if the umbrella file already has content, nothing is copied. Tests: 12 new pacing, 5 new migration, 4 new umbrella precedence on the existing modules; full guardian/services suite 179 passed.

For `v1.6.71`, the backend, frontend, Tauri shell, README, public download page, and release note are all advanced together so the installer, runtime self-inspection, and GitHub Pages downloader tell the same version story. This release tightens Sparkbot's self-learning loop in two ways. **Durable correction lock**: a new sidecar store at `backend/app/services/guardian/correction_lock.py` persists per-room intent-suppression flags so corrections like "that's not an answer" or "stop dumping state" survive past the original 6-message lookback window in `backend/app/api/routes/chat/rooms.py` and across sessions; explicit re-enable phrases ("yes show me the state", "actually display the stack") clear the lock for that intent. The store can be relocated via `SPARKBOT_CORRECTION_LOCK_DATA_DIR` and disabled via `SPARKBOT_CORRECTION_LOCK_ENABLED=false`. **Improvement proposals visible in Command Center**: three new operator-only routes (`GET /spine/operator/improvement/proposals`, `POST .../approve`, `POST .../reject`) back a new "Improvement" tab in the Spine and Guardian Inspector at the bottom of Command Center. The panel lists every proposal `guardian_propose_improvement` has recorded with status, risk, suggested change, evidence, and Approve / Reject controls; approving records operator intent only and never auto-applies a change. `update_proposal_status` in `improvement.py` stamps `approved_at`/`approved_by` or `rejected_at`/`rejected_by` so the loop has a paper trail. Tests: 9 new for the correction lock, 2 new for `update_proposal_status`, 158 services tests pass overall.

For `v1.6.70`, the backend, frontend, Tauri shell, README, public download page, and release note are all advanced together so the installer, runtime self-inspection, and GitHub Pages downloader tell the same version story. This release fixes the Command Center Inspector in the Tauri desktop build: every Spine and Guardian panel (Overview, Queues, Projects, Events, Producers, Security, Vault, Task Guardian, Project workload, Task distribution) was returning `Unexpected token '<', "<!DOCTYPE "... is not valid JSON` because `spineGet`, `guardianFetch`, and `roomsFetch` in `frontend/src/lib/spine.ts` used raw `fetch()` against `window.location.origin`. In the desktop build that origin is `tauri://localhost`, so requests hit the Tauri webview and got the SPA `index.html` fallback instead of the backend at `127.0.0.1:8000`. All three helpers now route through `apiFetch` from `apiBase.ts`, which resolves the desktop origin to the backend and injects the `chat_token` from sessionStorage as a Bearer header (the HttpOnly cookie cannot be sent cross-origin from the Tauri webview). Token Guardian "not registering" was a side-effect — `fetchGuardianStatus` was failing silently — so once the helpers work, the Token Guardian, Computer Control, Task Guardian, and SystemHealth cards in Command Center show real values again. The home dashboard route at `frontend/src/routes/_layout/index.tsx` had the same raw-fetch pattern for the dashboard summary and approval action endpoints; it was also replaced with `apiFetch`. Server-mode behaviour is unchanged — `apiFetch` returns same-origin paths when `getApiBase()` is empty.

For `v1.6.69`, the backend, frontend, Tauri shell, README, public download page, and release note were advanced together so the installer, runtime self-inspection, and GitHub Pages downloader tell the same version story. v1.6.69 fixed Token Guardian reporting: locked-provider and provider-authoritative routing payloads previously marked `live_ready: True` while bypassing Token Guardian, which made Command Center misreport active routing; they now expose `tg_bypassed: True` with a specific `tg_bypass_reason` and surface the underlying exception in `fallback_reason`. The chat audit log no longer records bypass payloads under the Token Guardian tool names, so the dashboard's 24-hour `live_routes_24h` counter accurately reflects actual Token Guardian decisions. `_model_is_configured` no longer permissively returns True for unknown model prefixes, the runtime version self-inspection no longer fabricates a `1.2.3` fallback, and Spine ingest failures from Token Guardian are now logged at warning level instead of silently swallowed.
...
│   │   │       ├── github.py              # GitHub event/API bridge
...
│   │   │   ├── github_bridge.py           # GitHub access and event bridge service

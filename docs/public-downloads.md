...
The current release is `v1.6.45` — governed Guardian Memory lifecycle and low-clutter recall.
...
bash scripts/package-public-download.sh \
  --ref desktop-v1.6.45 \
  --artifact-prefix sparkbot-1.6.45 \
  --output-dir dist/public-download/1.6.45
```
...
```bash
bash scripts/package-public-download.sh \
  --notes-file docs/release-notes/v1.6.45.txt
```

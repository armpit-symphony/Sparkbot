...
The current release is `v1.6.44` — break-glass everywhere and wider agent model selection.
...
bash scripts/package-public-download.sh \
  --ref desktop-v1.6.44 \
  --artifact-prefix sparkbot-1.6.44 \
  --output-dir dist/public-download/1.6.44
```
...
```bash
bash scripts/package-public-download.sh \
  --notes-file docs/release-notes/v1.6.44.txt
```

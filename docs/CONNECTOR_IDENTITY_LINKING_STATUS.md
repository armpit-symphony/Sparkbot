...
| Discord | Yes | Discord channel ID plus author ID/name/display name. | Creates an isolated `discord_<id>` Sparkbot user and bridge room per channel when first used. No explicit web-operator mapping in this pass. | Allowed for the linked bridge user/room only. | Possible for notes created under the same Discord-linked identity; not guaranteed for web operator meetings. | Partial fail-closed: guild restriction can block non-approved guilds; outgoing delivery has allowed-channel checks. Empty guild/channel allowlists permit isolated auto-linking. | SQLite `discord_links`; env `DISCORD_GUILD_IDS`, `DISCORD_ALLOWED_CHANNEL_IDS`, `DISCORD_DM_ONLY`. | Needs live sandbox QA before GREEN cross-surface meeting recall. |
| WhatsApp | Yes | WhatsApp `wa_phone` and display name. | Creates an isolated `whatsapp_<phone>` Sparkbot user and bridge room only after the inbound bridge is enabled with explicit verify token and `WHATSAPP_ALLOWED_PHONES`, and the phone is allowed. | Allowed for the linked phone identity/room only after the phone passes the allowed-phone gate; private meeting recall still requires PIN verification. | Possible for notes created under that linked identity; operator/private meeting recall requires connector PIN verification unless a stronger mapping is added later. | Fail-closed: missing `WHATSAPP_VERIFY_TOKEN` or empty `WHATSAPP_ALLOWED_PHONES` disables inbound registration; unlisted phones are rejected. | SQLite `whatsapp_links`; env `WHATSAPP_VERIFY_TOKEN`, `WHATSAPP_ALLOWED_PHONES`. | Needs live sandbox QA before GREEN cross-surface meeting recall. |

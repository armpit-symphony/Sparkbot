...
- [x] **Agent identity and kill switches.** Every agent exposes owner, purpose, scopes, allowed tools, expiration, risk tier, and kill-switch state.
- [x] **Run timeline evidence.** Dashboard timeline API returns room-scoped policy/tool events with agent/model attribution and audit hashes.
- [x] **Per-tool guardrails.** Input and output guardrails validate risky tool payloads before and after execution.
...
- [x] Publish a plain-language privacy and data retention page (non-technical users need this before trusting the product with their email/calendar).
- [x] Add visible permission labels in the UI/API — connector health now exposes read/write scopes before users connect or automate.
- [x] Add first-class agent identity records: owner, purpose, scopes, allowed tools, expiration, risk tier, and kill switch.
- [x] Add a trace timeline API for prompt-adjacent model/tool/approval/guardrail/output/audit-hash evidence.
- [x] Add a broader evaluation harness for agent behavior regressions.
...
- Connector health exposes setup state, scopes, setup-test flags, and audit metadata.
- Workflow templates cover morning brief, PR monitor, deploy checklist, inbox triage, and incident response.
- Governance evals exercise policy, guardrails, and agent routing.
...
- [x] Add clearer permission labels in the UI/API for non-technical users.
...
- [x] Add starter workflow templates for common governed jobs.
...
- [x] Add health/status surface for connector quality and Guardian governance evals.
...
- [x] Publish a plain-language privacy and data retention note.
...
2. Turn workflow templates into editable UI flows.
3. Build a dedicated visual trace viewer on top of the timeline API.
4. Add onboarding copy and product guardrails for first-time users.
5. Run an end-to-end consumer smoke pass on login, chat, reminders, Gmail, Drive, Telegram, meetings, approvals, and room settings.
